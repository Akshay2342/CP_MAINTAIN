...
void solve(){
    ...
    sort(all(v), sortcon);
    multiset<int> ms;
    ms.insert(v[0].first - v[0].second);
    int le=0;
    int maxi =0;
    for(int ri = 1; ri < n ;ri++){
        int pans = v[ri].fi + v[ri].se;
        while(v[ri].se - v[le].se > k){
            ms.erase(ms.find(v[le].fi - v[le].se));
            le++;
        }
        if(ms.size())
        maxi = max(maxi , *prev(ms.end()) + pans );
        ms.insert(v[ri].fi - v[ri].se);
    }
