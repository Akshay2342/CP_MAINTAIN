...
void solve() {
    ...
    vi lef;
    for(int i=0;i<l;i++) lef.pb(a[i]);
    if(lef.size() !=0) sort(all(lef));
    int m1=accumulate(all(curr) , 0LL);
    int pt=0;
    for(int i=0;i<min(lef.size() , curr.size());i++){
        if(lef[i] < curr[pt]) {
            m1 += (lef[i]-curr[pt]);
            pt++;
            continue;
    ...
    int m2 = accumulate(all(curr), 0LL);
    vi rig;
    for(int i=r+1;i<n;i++) rig.pb(a[i]);
    if(rig.size()) sort(all(rig));
    pt=0;
    for(int i=0;i<min(rig.size() , curr.size());i++){
        if(rig[i] < curr[pt]) {
            m2 += (rig[i]-curr[pt]);
            pt++;
            continue;
