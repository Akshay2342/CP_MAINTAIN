...
void solve(){
    ...
    for(auto it : ct){
        int rem = m;
        if(prevCost==INT_MAX || it.fi-prevCost > 1){
            prevCost = it.fi;
            prevNo = it.se;

            rem -= (it.fi * it.se);
            if(rem >= 0){
                ans = max(ans,it.se * it.fi);
            }else{
                ans = max(ans, (m/(it.fi))*it.fi );
            }
            continue;
        }
        rem -= (prevCost * prevNo);
        if(rem > 0){
         rem -= (it.fi * it.se);
            if(rem >= 0){
                ans = max(ans,it.se*it.fi+prevNo*prevCost);
            }else{
                ans = max(ans,prevNo*prevCost +  ((rem+it.fi*it.se)/(it.fi))*it.fi );
            }
        }
        int maxy= min(it.se , m/it.fi);
        ans = max(ans , maxy * it.fi);
        maxy = min(prevNo, m/prevCost);
        ans = max(ans, maxy * prevCost);

        rem =m;
        int b1 = min(prevNo, rem/prevCost);
        rem -= b1*prevCost;
        int b2 = min(it.se, rem/it.fi);
        ans =max(ans, min(m ,b1 * prevCost + b2 * it.fi + min(b1, it.se-b2)));
